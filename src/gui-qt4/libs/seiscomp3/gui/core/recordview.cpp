...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// bool RecordView::buildFilter(const QString& strFilter,
//                              std::vector<Seiscomp::Math::Filtering::IIR::Filter<float>* >* filterList) {
// 	QRegExp rx("^(\\w+)(?:\\()([,|\\.|\\d]+)(?:\\))$");
// 	if ( !rx.isValid() ) {
// 		std::cerr << "regexp error: " << rx.errorString().toStdString() << std::endl;
// 		return false;
// 	}
// 
// 	if ( strFilter.isEmpty() ) return false;
// 
// 	QStringList chainFilters = strFilter.split("->");
// 
// 	foreach ( QString filter, chainFilters ) {
// 		Seiscomp::Math::Filtering::IIR::Filter<float> *f = NULL;
// 
// 		QString filterType, filterParams;
// 	
// 		if ( rx.indexIn(filter, 0) != -1 ) {
// 			if ( rx.pos(1) != -1 )
// 				filterType = rx.cap(1);
// 			if ( rx.pos(2) != -1 )
// 				filterParams = rx.cap(2);
// 		}
// 	
// 		if ( filterParams.isEmpty() ) {
// 			std::cerr << "empty filter parameters for filter: " << filter.toStdString() << std::endl;
// 			return false;
// 		}
// 	
// 		QStringList params = filterParams.split(',');
// 		QVector<float> fParams;
...
// 				fParams.append(v);
// 			else {
// 				std::cerr << "wrong parameter format: " << s.toStdString() << std::endl;
// 				return false;
// 			}
// 		}
// 	
// 		if ( filterType == "BW" ) {
// 			if ( fParams.count() != 3 ) {
// 				std::cerr << "the butterworth filter needs 3 parameters, " << fParams.count() << "given" << std::endl;
// 				return false;
// 			}
// 	
// 			f = new Math::Filtering::IIR_ButterworthBandpass<float>((int)fParams[0],fParams[1],fParams[2]);
// 		}
// 		else if ( filterType == "AC" ) {
// 			if ( fParams.count() != 1 ) {
// 				std::cerr << "the amplitude accumulator filter needs 1 parameter, " << fParams.count() << "given" << std::endl;
// 				return false;
// 			}
// 	
// 			f = new Math::Filtering::IIR_RCAmp<float>(fParams[0]);
// 		}
// 		else if ( filterType == "STALTA" ) {
// 			if ( fParams.count() != 2 ) {
// 				std::cerr << "the sta lta filter needs 2 parameters, " << fParams.count() << "given" << std::endl;
// 				return false;
// 			}
// 
// 			f = new Math::Filtering::IIR::Op2Filter<float, std::divides>(
// 					new Math::Filtering::IIR_RCAmp<float>(fParams[0]),
// 					new Math::Filtering::IIR_RCAmp<float>(fParams[1]));
// 		}
// 		else {
// 			std::cerr << "unknown filter type: " << filterType.toStdString() << std::endl;
// 			return false;
// 		}
// 
// 		if ( f && filterList ) filterList->push_back(f);
// 	}
// 
// 	return true;
// }

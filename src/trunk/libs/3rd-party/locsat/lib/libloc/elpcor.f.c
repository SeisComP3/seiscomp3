...
int elpcor_(const char *phid, float *del, float *z__, float *azi, float *ecolat, float *ecorr, ftnlen phid_len) {
    /* Initialized data */
/*fprintf(stderr, "called elpcor_ %s %.3f %.1f %.3f %.3f %d\n", phid,*del,*z__,*azi, *ecolat, phid_len);
 */
    static float degrad = (float).0174532925;
    static float depth[8] = { (float)0.,(float)35.,(float)50.,(float)100.,(
	    float)200.,(float)300.,(float)500.,(float)700. };
    static float delinc = (float)5.;
    static float delmin[11] = { (float)0.,(float)0.,(float)145.,(float)145.,(
	    float)115.,(float)0.,(float)0.,(float)65.,(float)105.,(float)0.,(
	    float)110. };
    static float delmax[11] = { (float)100.,(float)90.,(float)175.,(float)155.,
...
#define tau ((real *)&equiv_23)
    ...
    /* Local variables */
    static int idel, idep;
    static float azir;
    static int idel1, idep1, i__;
    static float t[3], delta, dd, dz;
    static int ips;
    static float ecolatr, anumber;

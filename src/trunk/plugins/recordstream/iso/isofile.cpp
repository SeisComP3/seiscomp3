...
IsoFile::IsoFile()
: RecordStream()
, _extrDir("/tmp/seiscomp3")
, _readingData(false) {
	_recstream = new SDSArchive();
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool IsoFile::setSource(const string &src) {
	_name = src;
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool IsoFile::addStream(const string &net, const string &sta,
...
void IsoFile::extractFiles() {
	for ( _recstream->_curiter = _recstream->_streams.begin();
	      _recstream->_curiter != _recstream->_streams.end();
	      _recstream->_curiter++ ) {

		_recstream->_curidx = &*_recstream->_curiter;
		_recstream->setFilenames();

		while ( !_recstream->_fnames.empty() ) {
			string sdsname = _recstream->_fnames.front();
			_recstream->_fnames.pop();
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
Seiscomp::Record *IsoFile::next() {
	if ( !_readingData )
		extractFiles();

	return _recstream->next();
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

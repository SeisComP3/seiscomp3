...
class ConfigDBUpdater(seiscomp3.Client.Application):
  ...
  def createCommandLineDescription(self):
    ...
    self.commandline().addStringOption("Input", "key-dir", "Overrides the location of the default key directory ($SEISCOMP_ROOT/etc/key)")
    self.commandline().addGroup("Output")
    self.commandline().addStringOption("Output", "output,o", "If given, an output XML file is generated")
  ...
  def init(self):
    ...
    if len(self.bindingMods) == 0:
      sys.stderr.write("Warning: no usable modules found, nothing to do\n")
      return False
  ...
  def run(self):
    ...
    for cm in obsoleteConfigMods:
      sys.stderr.write("- %s / obsolete module configuration\n" % cm.name())
    ...
    if not configMod:
      ...
      if configMod is None:
        ...
        if configMod.name() != moduleName: configMod.update()
    ...
    if self._outputFile:
      ...
      if not ar.create(self._outputFile):
        sys.stderr.write("Failed to created output file: %s" % self._outputFile)
        return False
    ...
    if ncount > 0:
      ...
      if ncount > 1: sys.stderr.write("s")
      sys.stderr.write("\n")
    else:

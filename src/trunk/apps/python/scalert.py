...
class VoiceAlert(seiscomp3.Client.Application):
    ...
    def init(self):
        ...
        try:
            ...
            try:
                ...
                seiscomp3.Logging.warning("No amplitude script defined")

        if self._ampScript:
            self._ampScript = seiscomp3.System.Environment.Instance().absolutePath(self._ampScript)
            seiscomp3.Logging.info("Using amplitude script %s" % self._ampScript)
        try:
            ...
            try:
                ...
                seiscomp3.Logging.warning("No alert script defined")

        if self._alertScript:
            self._alertScript = seiscomp3.System.Environment.Instance(
        ...
        if self._eventScript:
            self._eventScript = seiscomp3.System.Environment.Instance(
        ...
        self._cache = seiscomp3.DataModel.PublicObjectRingBuffer(
            self.query(), 100)
        ...
        if self._agencyIDs:
            ...
            seiscomp3.Logging.info("agencyIDs: no filter is applied")
        ...
        try:
            ...
            if obj:
                if obj.type() == self._ampType:
                    seiscomp3.Logging.debug("got new %s amplitude '%s'" % (
            ...
            if obj:
                org = self._cache.get(
                    seiscomp3.DataModel.Origin, obj.preferredOriginID())
    ...
    def notifyEvent(self, evt, newEvent=True, dtmax=3600):
        try:
            org = self._cache.get(
                seiscomp3.DataModel.Origin, evt.preferredOriginID())
            if not org:
                seiscomp3.Logging.warning(
            ...
            dsc = seiscomp3.Seismology.Regions.getRegionName(
            ...
            if self._eventDescriptionPattern:
                try:
                    ...
                    if city:
                        ...
                        region = seiscomp3.Seismology.Regions.getRegionName(
                            org.latitude().value(), org.longitude().value())
    ...
    #       return
            ...
            try:
                ...
                seiscomp3.Logging.error("Failed to start event script '%s %s %d %d %s'" % (

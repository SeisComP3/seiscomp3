#!/usr/bin/perl
# Seismic PI station configuration utility.
# Writte by Kostas Boukouras (kbouk@noa.gr)
# Version 1.0. 20/12/2020.
#
# Check if SEISCOMP_ROOT variable is defined
$sr = `echo \$SEISCOMP_ROOT`; chomp $sr;

if ($sr ne "") {
#
# Shutting down SeisComP
print "Stopping SeisComP...\n";
`$sr/bin/seiscomp stop`;
...
$plugin_cfg = "$sr/var/lib/seedlink/$net.$station.ini";
print "\n\nMaking SeismicPI $plugin_cfg config file.\nPlease delete previous SeismicPI $sr/var/lib/seedlink/<NET>.<STATION>.ini files.\n\n";
